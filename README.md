...
[Mindustry](https://github.com/Anuken/Mindustry) Java mod jsonlib, otherwise known as JAR-modding, complete with [`EntityAnno`](https://github.com/GlennFolker/EntityAnno) and syntax downgrader integration, works for both Android and PC.

## Using

Before going into using this jsonlib, be aware that a fair amount of Java knowledge and Git *(GitHub Desktop is fine, but `git` CLI is a million times better)* is **highly beneficial**. Going in blind isn't impossible, but you'll face a lot of problems. Not that people on [the Discord](https://discord.gg/mindustry) won't help, though, so be communicative!
...
2. Click the `Use this jsonlib` button and create your repository.
...
4. Refactor namings to your preferences. The jsonlib is designed in such a way that this step should only require you to modify:
   - `gradle.properties`, the "Project configurations" section. For the "package" properties, if you don't know what you're doing, simply just change `jsonlib` to your preferred mod root package naming *(e.g. `mymod`, or `confictura` if your mod name is "confictura")*.
...
> [!TIP]
> There's no `name` property in this jsonlib's `mod.json`. That property is automatically filled up when building.

   Here's an example of a properly configured mod base from the jsonlib, *assuming
   ...
   -     "description": "Mindustry Java mod jsonlib, complete with EntityAnno and syntax downgrader integration.",
   +     "description": "Dive into the past of a trauma-driven uprising.",
         "version": "1.0",
         "minGameVersion": "146",
         "author": "You",
         "java": true,
   -     "main": "jsonlib.ModTemplate"
